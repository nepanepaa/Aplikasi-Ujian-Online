...
class Adm extends CI_Controller
{
	...
	public function m_kelas()
	{
		...
		if ($uri3 == "det") {
			...
			$q_datanya = $this->db->query("SELECT a.*
												FROM m_kelas a
												WHERE a.kelas LIKE '%" . $search['value'] . "%' ORDER BY a.id DESC LIMIT " . $start . ", " . $length . "")->result_array();
			$data = array();
			$no = ($start + 1);
	...
	public function m_ujian()
	{
		...
		if ($uri3 == "det") {
			...
			foreach ($q_datanya as $d) {
				...
				$data_ok[7] = '
		            	<div class="btn-group">
                          <a href="#" onclick="return m_ujian_e(' . $d['id'] . ');" class="btn btn-primary btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
